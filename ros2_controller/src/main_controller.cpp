 ...
 * @brief 프로그램 진입점 함수.
 *
 * ROS2 런타임을 초기화한 뒤 BatteryControllerNode를 생성하여 스핀한다.
...
int main(int argc, char* argv[]) {
  try {
    ...
    /** BatteryControllerNode를 생성 */
    auto node = std::make_shared<loadcell_controller::LoadcellControllerNode>();

    /** node 추가 */
    executor.add_node(node);

    /** BatteryControllerNode를 생성하여 콜백 처리를 시작한다. */
    executor.spin();
